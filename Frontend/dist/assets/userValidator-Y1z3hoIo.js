...
const planSchema = Joi.object({
  price: Joi.string().pattern(/^\d+$/).min(0).max(1e3).required().messages({
  ...
  discount: Joi.string().pattern(/^\d+$/).min(0).max(50).required().messages({
...
const profileEditSchema = Joi.object({
  ...
  pin_code: Joi.string().pattern(/^\d{6}$/).messages({
...
const reviewSchema = Joi.object({
  ...
  comment: Joi.string().max(500).pattern(
    ...
    "string.pattern.base": "Comment contains forbidden words. Please refrain from using offensive language.",
...
const passwordChangeSchema = Joi.object({
  ...
  newPassword: Joi.string().min(8).pattern(new RegExp("^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\\$%\\^&\\*])")).required().label("New Password").messages({
...
const passwordChangeValidator = (data) => {
...
const planValidate = (price, discount) => {
...
export {
  validate as a,
  validateEditing as b,
  planValidate as c,
  emailValidate as e,
  passwordChangeValidator as p,
  reviewValidator as r,
