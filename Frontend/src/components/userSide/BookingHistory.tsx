...
export default function BookingHistory() {
  ...
  const [workspaces, setWorkspaces] = useState <Partial<IWorkspace>>({});
  ...
  const fetchWorkspaceNames = async (bookings : IBookingDetails[]) => {
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      const response = await getBooking(user._id as string);
      if (response?.status !== 200) {
        toast.error("Something went wrong");
        return;
      }
  ...
  const handleCancelBooking = async (booking : IBookingDetails) => {
    try {
        ...
        <div className="relative overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {items.map((item : IBookingDetails, index) => (
                <tr
                  key={index}
