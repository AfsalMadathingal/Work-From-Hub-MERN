...
const ProfileSidebar = () => {
    ...
    const handlePhotoSave = async () => {
        ...
        try {
            const response = await editUserProfilePic(formData);  
            if (response.success) {
    ...
    <>
      ...
      <div className={`fixed z-50 inset-0 w-full  h-full bg-black bg-opacity-50 flex justify-center items-center ${editProfilePic ? 'block' : 'hidden'}`}>
        <div className="bg-white rounded-lg p-4 w-2/6 text-center flex flex-col justify-center items-center relative">
          ...
          <div className="mt-4 ">
            <img src={profilePic ? profilePic : user?.profilePic} className="w-40 h-40 rounded-full object-full mx-auto" alt="" />
            <div className="mt-4 flex flex-col justify-center items-center">
              ...
              className='bg-orange-500 py-2 px-4 rounded-md text-white font-bold mt-4'>{loading ? <Loading type="spin" color="white" height={20} width={20} /> : "Save"}</button>
     ...
     <aside className={`bg-white shadow-md mt-5 ms-56 rounded-lg p-4 transition-transform ${isSidebarOpen ? 'transform-none' : '-translate-x-full lg:translate-x-0'} lg:w-60 lg:block`}>
          <div className="flex   flex-col justify-center items-center ">
            <img  src={user?.profilePic} className="w-20 h-26 rounded-full object-cover" alt="" />
