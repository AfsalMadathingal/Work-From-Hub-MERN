...
const seatModel_1 = require("../../models/seatModel");
class SeatRepository {
    async createSeatsForWorkspace(workspace) {
        try {
            console.log("====================================");
            ...
            for (let tableNumber = 1; tableNumber <= tablesAvailable; tableNumber++) {
                for (let seatNumber = 1; seatNumber <= seatsPerTable; seatNumber++) {
                    ...
                    seatPromises.push(seatModel_1.Seat.create(newSeat));
                }
            }
    ...
    async getSeatsByWorkspaceId(workspaceId) {
        try {
            const seats = await seatModel_1.Seat.find({ workspaceId });
            return seats; // This will include the availability field
        }
        catch (error) {
            console.error(error);
            return null;
    ...
    //   }
    // }
    async reserveSeat(workspaceId, tableNumber, seatNumber, selectedDate) {
        try {
            // Fetch the specific seat
            const seat = await seatModel_1.Seat.findOne({ workspaceId, tableNumber, seatNumber });
            if (!seat) {
                throw new Error("Seat not found");
            }
            // Update seat availability for the selected date
    ...
    async checkIfSeatIsReserved(seatId, date) {
    ...
    async getSeatById(seatId) {
        try {
            const seat = await seatModel_1.Seat.findById(seatId);
            if (!seat) {
                throw new Error("Seat not found");
            }
            return seat;
    ...
    async listSeatsWithAvailabilityCheck(workspaceId, date) {
        ...
        const seatsWithAvailability = await Promise.all(seats.map(async (seat) => {
            const isAvailable = await this.isSeatAvailableForDate(seat._id, date);
            return {
                seatId: seat._id,
    ...
    async isSeatAvailableForDate(id, date) {
    ...
    async bookSeatById(workspaceId, seatId, date) {
    ...
    async bookSeat(workspaceId, seatId, selectedDate) {
        return null;
    }
}
