...
export interface IWorkspaceRepository {
  ...
  getWithFilters( query:string,filter:Partial<IFilters>  , page:number,limit:number , sortOrder:{}):Promise<{ Workspaces: IWorkspace[]; totalPages: number; } | null>;
  ...
  findByIdAndUpdate(workspaceId: string, data: Partial<IWorkspace>): Promise<IWorkspace | null>;
  unHoldWorkspace(workspaceId:string) :Promise <IWorkspace | null > ;
}